...
public class TerminalPanel extends JComponent implements TerminalDisplay, TerminalActionProvider {
    ...
    protected volatile Dimension myTermSize = new Dimension(80, 24);
    private volatile long lastModifySizeTimeMillis = 0;
    ...
    public Dimension requestResize(final Dimension newSize,
                                   ...
                                   int cursorY,
                                   JediTerminal.ResizeHandler resizeHandler) {
        Dimension dimension = new Dimension(getPixelWidth(), getPixelHeight());
        //修改尺寸调整问题
        if (!newSize.equals(myTermSize) && TimeUnit.MILLISECONDS.toMillis(System.currentTimeMillis() - lastModifySizeTimeMillis) >= 300) {
            lastModifySizeTimeMillis = System.currentTimeMillis();
            ...
            dimension = new Dimension(getPixelWidth(), getPixelHeight());
            setPreferredSize(dimension);
            if (myTerminalPanelListener != null) {
                myTerminalPanelListener.onPanelResize(dimension, origin);
            }
...
//            System.out.println("File show length:" + blockLen + "&getBuf().length：" + renderingBuffer.getBuf().length);
//            gfx.drawChars(renderingBuffer.getBuf(), buf.getStart() , renderingBuffer.getBuf().length, xCoord, baseLine);
