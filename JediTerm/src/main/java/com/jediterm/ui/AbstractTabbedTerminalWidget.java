...
public abstract class AbstractTabbedTerminalWidget<T extends JediTermWidget> extends JPanel implements TerminalWidget, TerminalActionProvider {
    ...
    private void addTab(T terminal, AbstractTabs<T> tabs) {
        addTab(terminal, tabs, false);
    }

    private void addTab(T terminal, AbstractTabs<T> tabs, boolean firstTab) {
    ...
    @NotNull
    String getTabName(@NotNull T terminal) {
    ...
    private void addTab(T terminal, AbstractTabs<T> tabs, String name, boolean firstTab) {
        tabs.addTab(name, terminal);

        tabs.setTabComponentAt(tabs.getTabCount() - 1, createTabComponent(tabs, terminal, firstTab));
        tabs.setSelectedComponent(terminal);
    }

    private void addTab(T terminal, AbstractTabs<T> tabs, String name) {
        addTab(terminal, tabs, name, false);
    }
    ...
    private AbstractTabs<T> setupTabs(T terminal) {
    ...
    protected Component createTabComponent(AbstractTabs<T> tabs, T terminal, boolean firstTab) {
        return new TabComponent(tabs, terminal, firstTab);
    }
    ...
    public void closeTab(final T terminal) {
        if (terminal != null) {
            SwingUtilities.invokeLater(() -> {
                        removeTab(terminal);
                        System.out.println("当前大小:" + myTabs.getTabCount());
                        if (myTabs.getTabCount() < 1) {
                            System.out.println("调用创建新组件");
                            //至少保留一个Tab组件
                            handleNewSession();
//                            myTabs.setSelectedIndex(0);
    ...
    public List<TerminalAction> getActions() {
        return List.of(new TerminalAction(mySettingsProvider.getNewSessionActionPresentation(), input -> {
            handleNewSession();
            return true;
        }).withMnemonicKey(KeyEvent.VK_N), new TerminalAction(mySettingsProvider.getCloseSessionActionPresentation(), input -> {
            closeCurrentSession();
            return true;
        }).withMnemonicKey(KeyEvent.VK_S), new TerminalAction(mySettingsProvider.getNextTabActionPresentation(), input -> {
            selectNextTab();
            return true;
        }).withEnabledSupplier(() -> myTabs != null && myTabs.getSelectedIndex() < myTabs.getTabCount() - 1), new TerminalAction(mySettingsProvider.getPreviousTabActionPresentation(), input -> {
            selectPreviousTab();
            return true;
    ...
    private class TabComponent extends JPanel implements FocusListener {
        ...
        private TabComponent(final @NotNull AbstractTabs<T> tabs, final T terminal, boolean firstTab) {
