...
//        }
  ...
  protected int doPaintText(Graphics2D g, int offset, boolean focusAroundIcon) {
    ...
    for (int i = 0; i < myFragments.size(); i++) {
      ...
      if (attributes.isBoldDottedLine()) {
        ...
        DrawUtil.drawBoldDottedLine(g, offset, offset + fragmentWidth, dottedAt, bgColor, lineColor, isOpaque());
      }
    ...
    for (final Object[] info : searchMatches) {
      ...
      DrawUtil.drawSearchMatch(g, x1, x2, getHeight());
  ...
  protected class AccessibleSimpleColoredComponent extends AccessibleJComponent {
    @Override
